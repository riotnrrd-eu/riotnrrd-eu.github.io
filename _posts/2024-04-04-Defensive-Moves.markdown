...
The most notable vendor to do so was Elastic, in 2021, finally prompting the OSI to [opine officially](https://opensource.org/blog/the-sspl-is-not-an-open-source-license) on the open-sourceness of the SSPL (tl;dr: "no"). Elastic arguably left its move too late, as AWS and others were able to fork the codebase as it stood prior to the relicensing and create the [OpenSearch](https://opensearch.org) distribution, which AWS now uses to power its own services — without contributing back to Elastic, of course.[^2]
